...
# Define  parser
def arg_parser():
    parser = argparse.ArgumentParser("Neural network settings")
    parser.add_argument('--image', type = str, help= 'Point to image file')
    parser.add_argument('--checkpoint',  type = str, default=None,  help='Model to use when predicting')
    parser.add_argument('--top_k', action='store', type=int, default=5,  help='Top K matches as int')
    parser.add_argument('--category_names', type=str, default="cat_to_name.json", help='mapping categories to names')
#    parser.add_argument('--labels', type=str, help='cat_to_name.json')
    parser.add_argument('--gpu',  action='store_true', help='Use GPU + Cuda calculations')
...
def load_checkpoint(checkpoint_path):
    checkpoint = torch.load("mlw_checkpoint_pth")
